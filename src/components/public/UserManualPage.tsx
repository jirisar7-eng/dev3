...
export const UserManualPage: React.FC<UserManualPageProps> = ({ onNavigate }) => {
  ...
  const getIcon = (type: string) => {
    switch(type) {
      case 'play': return <PlayCircle className="w-6 h-6 text-blue-600" />;
      case 'file': return <FileText className="w-6 h-6 text-blue-600" />;
      case 'shield': return <ShieldCheck className="w-6 h-6 text-blue-600" />;
      case 'lifebuoy': return <LifeBuoy className="w-6 h-6 text-blue-600" />;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading ? (
          <div className="text-center py-20">
            <p className="text-slate-500 font-bold">Načítám manuál...</p>
          </div>
        ) : (
            ...
            {filteredData.map(section => (
              <div key={section.id} className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-6 space-y-3">
                  {section.articles.length === 0 ? (
                    <p className="text-sm text-slate-400">Žádné články nenalezeny.</p>
