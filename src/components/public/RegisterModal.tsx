...
export const RegisterModal: React.FC<RegisterModalProps> = ({
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const childrenData = fillProfileNow || hasChildrenInitial
        ? children
                        ...
                        {children.map((child, index) => (
                          <div key={index} className="p-3 bg-slate-50 rounded-2xl border border-slate-200 relative space-y-2">
                            ...
                            <div className="flex items-center gap-2 pt-1">
                              <input
                                type="checkbox"
                                id={`modal-isStudying-${index}`}
                                checked={child.isStudying || false}
                                onChange={(e) => handleChildChange(index, 'isStudying', e.target.checked)}
                                className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
                              />
                              <label htmlFor={`modal-isStudying-${index}`} className="text-xs text-slate-700 cursor-pointer select-none">
