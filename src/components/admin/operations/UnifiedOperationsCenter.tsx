...
export const UnifiedOperationsCenter: React.FC<UnifiedOperationsCenterProps> = ({
  ...
  const [syncingNotion, setSyncingNotion] = useState<boolean>(false);
  const [syncResult, setSyncResult] = useState<any>(null);

  const fetchKnowledgeData = async () => {
    try {
      const [resK, resS] = await Promise.all([
        apiFetch('/api/admin/qa/knowledge'),
        apiFetch('/api/admin/qa/knowledge-mirror/status')
      ]);
      const jsonK = await resK.json();
      const jsonS = await resS.json();
      if (jsonK.success) setKnowledgeRecords(jsonK.records || []);
      if (jsonS.success) setNotionStatus(jsonS.status);
    } catch (err) {
  ...
  const handleSyncNotion = async () => {
    ...
    setSyncResult(null);
    try {
      const res = await apiFetch('/api/admin/qa/knowledge-mirror/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scope: 'ALL', forceResync: false }),
      });
      ...
      if (json.success) {
        ...
        setSyncResult({ message: json.error || 'Sync selhal.' });
      }
    } catch (err: any) {
      setSyncResult({ message: err.message || 'Chyba při komunikaci se serverem.' });
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-px overflow-x-auto">
        ...
        <button
          onClick={() => setActiveSubTab('governance_knowledge')}
          className={`inline-flex items-center gap-2 px-4 py-3 text-xs font-bold border-b-2 transition-all whitespace-nowrap cursor-pointer ${
            activeSubTab === 'governance_knowledge'
              ? 'border-indigo-600 text-indigo-600 bg-indigo-50/50 rounded-t-xl'
              : 'border-transparent text-slate-600 hover:text-slate-900 hover:border-slate-300'
          }`}
        >
      ...
      {/* 10. Tab Content: GOVERNANCE & KNOWLEDGE MIRROR */}
      {activeSubTab === 'governance_knowledge' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4 mb-4">
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-bold text-slate-900">Notion Read-Only Knowledge Mirror</h2>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${notionStatus?.enabled ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                    {notionStatus?.enabled ? 'Aktivní Zrcadlo' : 'Izolovaný Lokální Režim'}
                  </span>
                </div>
                <p className="text-xs text-slate-500 mt-1">
                  {notionStatus?.message || 'Kontrola stavu Notion API propojení...'}
                </p>
              </div>
            ...
            {syncResult && (
              <div className={`p-3 rounded-xl mb-4 text-xs font-medium ${syncResult.failedCount === 0 ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-amber-50 text-amber-800 border border-amber-200'}`}>
                {syncResult.message}
              </div>
            )}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-slate-500 uppercase px-2">
                <span>Položka Knowledge Base ({knowledgeRecords.length})</span>
                <span>Klasifikace & Trust Level</span>
              </div>

              {knowledgeRecords.length === 0 ? (
                <div className="p-8 text-center text-slate-500 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-xs">
                  Žádné knowledge záznamy nebyly načteny.
                </div>
              ) : (
                <div className="divide-y divide-slate-100 border border-slate-200 rounded-2xl overflow-hidden bg-white">
                  {knowledgeRecords.map((item) => (
                    <div key={item.id} className="p-4 hover:bg-slate-50/80 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
                      <div className="space-y-1 max-w-2xl">
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded-md font-extrabold text-[10px] uppercase border ${
                            item.type === 'VERIFIED_FACT' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                            item.type === 'AI_RECOMMENDATION' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' :
                            item.type === 'AUDIT_FINDING' ? 'bg-rose-50 text-rose-700 border-rose-200' :
                            item.type === 'DRAFT_ACTION' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                        ...
                        <div className="flex items-center gap-3 text-[10px] text-slate-400 font-mono pt-1">
                          <span>Oblast: {item.projectArea}</span>
                          <span>SHA: {item.sourceCommitSha}</span>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <span className="font-mono text-[10px] text-slate-400 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200" title="Deterministic Content Hash">
