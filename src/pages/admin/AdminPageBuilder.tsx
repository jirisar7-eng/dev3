...
export const AdminPageBuilder: React.FC<{ onNavigate?: (path: string) => void }> = ({ onNavigate }) => {
  ...
  if (isLoading) return <div className="p-20 text-center text-slate-900 bg-white h-screen">Načítám stránku v Puck editoru...</div>;

  return (
    <div className="fixed inset-0 z-[9999] flex flex-col w-screen h-screen bg-white overflow-hidden">
      {/* Top Bar */}
      <div className="h-14 bg-slate-50 border-b border-slate-200 px-4 flex items-center justify-between z-10 shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/admin/pages')} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 transition-colors cursor-pointer">
            <ArrowLeft className="w-5 h-5" />
            <span className="text-sm font-medium">Zpět</span>
          </button>
          <div className="flex items-center gap-2">
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Název..." className="bg-transparent text-slate-900 font-bold text-sm focus:outline-none w-32 md:w-48" />
      ...
      {isTemplateModalOpen && (
        <div className="fixed inset-0 z-[10000] bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl text-slate-900 shadow-2xl p-6 max-h-[85vh] flex flex-col">
            <div className="flex justify-between items-center pb-4 border-b border-slate-200 mb-4">
              ...
              <button onClick={() => setIsTemplateModalOpen(false)} className="text-slate-400 hover:text-slate-900 text-xl font-bold cursor-pointer">&times;</button>
            </div>
            ...
            {!saveAsTemplateMode ? (
              <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                {isLoadingTemplates ? (
                  ...
                  templates.map((tpl) => (
                    <div key={tpl.id} className="p-4 bg-slate-950 border border-slate-200 rounded-xl flex items-center justify-between gap-4 hover:border-slate-200 transition-colors">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-bold text-slate-900">{tpl.name}</span>
                          <span className="px-2 py-0.5 bg-slate-50 text-indigo-300 font-bold text-[10px] rounded uppercase">{tpl.category}</span>
                        </div>
              ...
              <form onSubmit={handleSaveAsTemplate} className="space-y-4 my-auto">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Název nové šablony *</label>
                  <input
                    type="text"
                    required
                    value={templateName}
                    ...
                    className="w-full p-2.5 bg-slate-950 border border-slate-200 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 bg-slate-950 border border-slate-200 rounded-xl text-sm text-slate-900"
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Stručný popis</label>
                  <textarea
                    value={templateDescription}
                    onChange={(e) => setTemplateDescription(e.target.value)}
                    rows={3}
                    placeholder="Popište prvky obsažené v této šabloně..."
                    className="w-full p-2.5 bg-slate-950 border border-slate-200 rounded-xl text-xs text-slate-900"
                  />
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    type="button"
                    onClick={() => setSaveAsTemplateMode(false)}
                    className="px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-xs"
                  >
                    Zpět
                  </button>
                  <button
      ...
      {isAiModalOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 p-4">
          <div className="bg-slate-50 p-6 rounded-2xl w-full max-w-lg border border-slate-200 shadow-2xl">
            <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-purple-400" />
              <span>Generovat stránku pomocí AI</span>
            </h2>
            <input value={aiTitle} onChange={(e) => setAiTitle(e.target.value)} placeholder="Název článku nebo témata..." className="w-full p-2.5 mb-4 bg-white border border-slate-200 text-slate-900 rounded-xl text-sm"/>
