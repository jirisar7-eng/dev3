...
export class UserDataService {
  ...
  static async createChild(
    requestingUser: User,
    data: { userId?: string; name?: string; firstName?: string; lastName?: string; birthDate?: string; notes?: string }
  ): Promise<UserChild> {
    const userId = data.userId || requestingUser.id;
    if (!this.isOwnerOrAdmin(userId, requestingUser)) {
  ...
  static async updateChild(
    requestingUser: User,
    childId: string,
    data: { name?: string; firstName?: string; lastName?: string; birthDate?: string; notes?: string }
  ): Promise<UserChild> {
    if (prisma) {
      try {
