...
Požadované JSON schéma (vracej POUZE tento JSON, žádný jiný text, žádný markdown):
{
  ...
  private static async parseWithVision(file: Express.Multer.File, sourceDocumentId: string): Promise<JudgmentExtractedData> {
    ...
    // 1. Try Primary Gemini Key (gemini-1.5-flash)
    if (primaryKey) {
      try {
        console.log('[JudgmentParserService] Invoking Vision OCR with Primary GEMINI_API_KEY (gemini-1.5-flash)...');
    ...
    if (secondaryKey) {
      try {
        console.log('[JudgmentParserService] Invoking Vision OCR with Secondary GEMINI_API_KEY_2 (gemini-1.5-flash)...');
