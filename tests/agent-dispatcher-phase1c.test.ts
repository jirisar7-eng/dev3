...
describe('Phase 1C — Safe Agent Dispatcher', () => {
  ...
  it('15. registered handler (DOCUMENT_PROCESSOR:document.read) executes after authorization', async () => {
    const devUser = { id: 'u_123', role: 'ADMIN' };
    
    // 1. Verify handler is registered for capability in dispatcher
    const registeredHandler = (AgentDispatcher as any).handlers.get('DOCUMENT_PROCESSOR:document.read');
    expect(registeredHandler).toBeDefined();

    // 2. Verify authorization occurs before execution and dispatches to handler
    const result = await AgentDispatcher.dispatch({
      agentId: 'DOCUMENT_PROCESSOR',
      capabilityId: 'document.read',
      payload: { documentId: 'non_existent_doc_test15' },
      user: devUser as any
    });
    expect(result.decision).toBe('ALLOW');
    expect(result.reason).not.toContain('No execution handler registered');
